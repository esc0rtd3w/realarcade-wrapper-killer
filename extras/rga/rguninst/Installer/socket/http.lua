...
-- LuaSocket toolkit.
-- Author: Diego Nehab
-- RCS ID: $Id: http.lua,v 1.2 2006/04/22 18:13:36 twilliams Exp $
-----------------------------------------------------------------------------
...
function metat.__index:sendheaders(headers, reqline)
    local h = "\r\n"
...
local function adjustheaders(headers, host, redirects)
    -- default headers
    local lower = {
    ...
    -- override with user headers (but not if we're on a redirect)
    if redirects == 0 then
...
local function adjustrequest(reqt)
    ...
    nreqt.headers = adjustheaders(nreqt.headers, nreqt.host, reqt.nredirects )
    return nreqt
end
...
function trequest(reqt)
    ...
    local reqline = h:sendrequestline(reqt.method, reqt.uri)
    h:sendheaders(reqt.headers, reqline)
