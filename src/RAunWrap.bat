@echo off
title RealArcade Wrapper Killer [Getting Version Info...]    (.-+'~^-+ Demo unWrapper +-^~`+-.)     [...cRypTiCwaRe 2o13...]
...
::if %ghFlag%==1 call "core_check_non_rgs.bat"
::if %ghFlag%==1 call "core_check_exit_status.bat"


if exist "iAmZylom.tmp" set zyFlag=1

if %zyFlag%==1 set forceExit=1
::if %zyFlag%==1 call "core_check_non_rgs.bat"
::if %zyFlag%==1 call "core_check_exit_status.bat"
...
call "core_check_local_rawk.bat"
...
if "%cd%" == "C:\My Games" call "core_invalid_folder.bat"
...
inifile "%rawkroot%\rawkdb.ini" [core] rdb_core > dbtemp.bat
call dbtemp.bat
...
if %rauninstall%==1 call "core_config_ra_player.bat"
...
if %advancedloadoptions%==1 call "core_advanced_load_options.bat"
...
call "core_reg_check.bat"

::-----------------------------------------------------------------------------------
:: Checking Reset Flag
::-----------------------------------------------------------------------------------

if %reset%==1 call "core_reset.bat"

::-----------------------------------------------------------------------------------
:: Checking Clear Cache Flag
::-----------------------------------------------------------------------------------

if %clearcache%==1 call "core_clear_cache.bat"
...
call "core_disable_old_reg_hacks.bat"
...
call "core_rgs_unlock_license.bat"
...
call "core_multiple_exe_check.bat"
...
if %rgspre%==1 call "core_rgs_pre_release.bat"
...
if "%GameName%"=="My Games" call "core_invalid_folder.bat"

::-----------------------------------------------------------------------------------
:: Creating Backup if no RAR file exists, but skipping backup if a RAR file exists
::-----------------------------------------------------------------------------------
if exist *.rar goto bakfound
if not exist *.exe call "core_no_game_found.bat"

if %quw%==0 call "core_backup.bat"
...
if %wrestore%==1 call "core_restore.bat"
...
:rewrap
if not exist *.exe call "core_restore.bat"
if %restartcore%==1 goto restart0
...
cls
inifile "%rawkroot%\rawkdb.ini" [%wrapped2%] unwrapped > unwrapcheck.bat
call "unwrapcheck.bat"
if exist "%unwrapped%.exe" call "core_game_unpacked_ok.bat"
...
:iniread
call "core_get_stats.bat"

::-----------------------------------------------------------------------------------
:: Workaround if there are no variables set.
:: It checks ALL entries from the status.bat, then assigns "0" to entry if the entry
:: tag is empty.
:: IF THIS IS NOT DONE, IT WILL CRASH IF THE GAME DOES NOT HAVE A SECTION IN THE
:: INI FILE.
::-----------------------------------------------------------------------------------

call "core_no_var_fix.bat"
...
if not %altid%==None call "core_dupe_check.bat"
...
if %quw%==0 call "core_show_stats.bat"
...
if %dtor%==1 call "core_demo_type_override.bat"
...
if %forceExit%==0 call "core_ini_output.bat"
if %forceExit%==0 exit

:: Only triggered by a Gamehouse or Zylom game
if %ghFlag%==1 call "core_gamehouse_main.bat"
::if %ghFlag%==1 call "core_gamehouse_hotkey.bat"
::if %ghFlag%==1 call "core_gamehouse_override.bat"

if %zyFlag%==1 call "core_zylom_main.bat"
::if %zyFlag%==1 call "core_zylom_hotkey.bat"
::if %zyFlag%==1 call "core_zylom_override.bat"


:: Needed for CLI launch Only
::call "core_rawk_cleanup_loader.bat"
if %forceExit%==1 call "core_rawk_cleanup_root.bat"
